...
public class BoardManager {
	...
	public ArrayList<CellStatus[][]> getBoardStates() {
		return boardStates;
	}
	
	public void setBoardStates(ArrayList<CellStatus[][]> newBoardStates){
		//test
		boardStates.removeAll(boardStates);
		for (CellStatus[][] a : newBoardStates){
			boardStates.add(a);
		}
		CellStatus[][] boardTmp=boardStates.get(boardStates.size()-1);
		for(int i=0;i<OthelloConstants.HEIGHT;i++){
			for(int j=0;j<OthelloConstants.WIDTH;j++){
				setCellStatus(i,j,boardTmp[i][j]);
			}
		}
	}
}
